...
void AppWindow::glutKeyboard ( unsigned char key, int x, int y )
 {
   switch ( key )
    {	case ' ': _viewaxis = !_viewaxis; redraw(); break;
		case '1': _ctype = LAGRANGE; redraw(); break;
		case '2': _ctype = BEZIER; redraw(); break;
		case '0': _ctype = NOCURVE; redraw(); break;
		case '3': _ctype = BSPLINE; redraw(); break;
		case '4': _ctype = CRSPLINE; redraw(); break;
		case '5': _ctype = BOSPLINE; redraw(); break;
		case 'q': numSegs += 1; redraw(); break;
		case 'a': numSegs -= 1; redraw(); break;
		case 'x': _xType = (_xType == D_OFF) ? D_ON : D_OFF; redraw(); break;
		case 'z': _zType = (_zType == D_NONE) ? D_ONCE : D_NONE; redraw(); break;
...
void AppWindow::glutDisplay ()
 {
   ...
   if (_ctype != NOCURVE) {
	   _curve.build(numSegs, _poly.vertexarray(), _ctype);
	   _curve.draw(_str, _spr);
	   if (_xType == D_ON) {
		   _draw3D.build(_curve.getCopy());
	   }
	   else if (_zType == D_ONCE) {
		   _zType = D_NONE;
		   _draw3D.build(_curve.getCopy());
	   }
	   _draw3D.draw(_str, _spr);
   }
