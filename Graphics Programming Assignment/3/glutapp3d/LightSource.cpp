...
void LightSource::init(const GlProgram& prog)
{
...
void LightSource::build(float x, float y, float z)
{
	P.clear(); C.clear(); // set size to zero, just in case
	P.reserve(1); C.reserve(1); // reserve space to avoid re-allocations from the calls below

	P.push_back(GsVec(x, y, z)); C.push_back(GsColor::lightblue);
...
void LightSource::draw(GsMat& tr, GsMat& pr)
{
	// Draw Lines:
	glUseProgram(prog);
