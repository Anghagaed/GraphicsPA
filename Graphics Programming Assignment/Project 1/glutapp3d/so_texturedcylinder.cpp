...
void SoTexturedCylinder::setMyTrans(const GsMat& transMat) {
	myTrans = transMat;
}
...
void SoTexturedCylinder::buildBody() {
...
void SoTexturedCylinder::buildHat() {
	for (int i = 0; i < dx.size() - 1; ++i) {
		GsPnt a, b, c;
		GsVec n;
		a = GsPnt(x				   , y + l / 2, z);
...
void SoTexturedCylinder::build(float r, float l, int _nfaces)
{
	T.clear();
