...
// Draws Tube
void SoTexturedFloor::drawTube(double radius, double height, double sine_r, double cosine_r, double sine_l, double cosine_l)
{
	// Get a,b,c,d
	// Get top coordinates
...
// Draws Circle
void SoTexturedFloor::drawCaps(double radius, double height, double sine_r, double cosine_r, double sine_l, double cosine_l) //, int frame_num, double circle_rate, double radius, double curr_angle_r, double curr_angle_l)
{
	// Get top coordinates
	// side of the rectangles in the tube setion
	double x_r = radius * sine_r;
...
void SoTexturedFloor::drawPrism(double radius)
{
	int const nfaces = 2;									// 4 faces = rectangular prism
	// Variables
	if (radius < 0)
	{
	...
	for (int i = 0; i < nfaces; i++)
	{
		...
		drawTube(radius, height, sine_r, cosine_r, sine_l, cosine_l); // draws tube
...
void SoTexturedFloor::build(double radius)
{
	P.size(0); T.size(0); N.size(0); NL.size(0); // set size to zero, just in case

	drawPrism(radius); // draws "Capsule"
